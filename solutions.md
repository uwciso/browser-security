...
```
Of the two methods to allow inline scripts (which, again, is not advisable), the first is preferable, in that it allows two specific inline scripts by supplying their hashes. Any other inline scripts would be blocked. Using the "unsafe-inline" method is, of course, unsafe because it opens the page up to *any* inline script.
...
```
This is a CORS request, since a crossorigin="anonymous" attribute is included on the script tag, and the jQuery CDN provided an "Access-Control-Allow-Origin: *" header. You can verify CORS by inspecting the request for an "Origin" header.
